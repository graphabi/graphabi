...
The production correction keeps the Graph Playground as the only explanatory graph. Its semantic pulse travels from producer to consumer, stops at the broken edge, and opens a witness. A separate Canvas field is permitted behind the website because it never asserts a product result: pointer velocity may gather, separate, and reroute neutral topology, then release it to equilibrium. The field remains decorative and `aria-hidden`; it never displays pass, break, uncertainty, witness, impact, or repair states. Reports never contain it.
